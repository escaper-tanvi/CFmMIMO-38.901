classdef hPre6GAPLowPHY < nr5g.internal.nrGNBPHY
    %hPre6GAPLowPHY Implements pre6G access point (AP) lower PHY functionality
    ...
    properties (SetAccess = private, Hidden)
        %Split Specify the Split as "Centralized", "Distributed", or "7.2x".
    ...
    methods
        function obj = hPre6GAPLowPHY(param, notificationFcn)
        ...
        function addConnectionToCPU(obj, param)
            ...
            slotDuration = 1/(param.SubcarrierSpacing/15);
            obj.SlotDurationInNS = 1/(param.SubcarrierSpacing/15) * 1e6; % In nanoseconds
            ...
            obj.PUSCHInfo.PUSCHConfig.NID = hPre6GAP.getCPUCellID(obj.APCellID);

            % Set centralized or distributed split
            ...
            obj.NextRxTime = Inf(symbolsPerFrame, 1);
        ...
        function nextInvokeTime = run(obj, currentTime, packets)
            ...
            % Run MAC operations only once in the same symbol
            ...
            % Segregate packets based on the type
    ...
    methods(Access=private, Hidden)
            ...
            for idx=1:length(srsConfig)
                if isa(srsConfig(idx), 'nrSRSConfig')
                    if ischar(srsConfig(idx).SRSPeriod)
                        ...
                        % represented as periodicity and slot offset
                        count = count + 1;
                        srsInfo(count, 1) = srsConfig(idx).SRSPeriod(1) * obj.SlotDurationInNS;
                        srsInfo(count, 2) = srsConfig(idx).SRSPeriod(2) * obj.SlotDurationInNS;
                    end
                end
            end
        ...
        function dataRxMAC(obj, currentTimeInfo)
            %dataRxMAC Send Rx start request to PHY for the receptions scheduled to start now

            if ~isempty(obj.UplinkRxContext)
                ulRxContext = obj.UplinkRxContext(:, (currentTimeInfo.NSlot * nr5g.internal.nrMAC.NumSymbols) + currentTimeInfo.NSymbol + 1); % Rx context of current symbol
                txUEs = find(~cellfun(@isempty, ulRxContext)); % UEs which are assigned uplink grants starting at this symbol
                for i = 1:length(txUEs)
                    % For the UE, get the uplink grant information
                    uplinkGrant = ulRxContext{txUEs(i)};
        ...
        function ulControlRequestMAC(obj, currTimingInfo)
            ...
            if ~isempty(obj.SRSConfiguration) % Check if SRS is enabled
                % Check if current slot has UL symbols. For FDD
    ...
    methods(Access=protected)
        function processControlPackets(obj, currTimeInfo, controlPktsDL, controlPktsUL)
            ...
            for i=1:size(controlPktsDL, 1)
                ...
                packet.DirectToDestination = packet.Metadata.DirectID;
            ...
            for i=1:size(controlPktsUL, 1)
                ...
                packet.Metadata.NCellID = hPre6GAP.getCPUCellID(obj.APCellID);
        ...
        function pdschTx(obj, pdschPkts)
            ...
            if strcmp(obj.Split, "Centralized")
                ...
                for i=1:length(pdschPkts)
                    packet = pdschPkts(i);
                    % Signal amplitude. Accounts for FFT occupancy factor
    ...
    methods(Hidden)
        ...
        % Do not create CSI data. Overwriting csirsData method implemented in nrGNBFullPHY  
        function csirsData(~, ~); end

        % Do not create CSI packet. Overwriting csirsPacket method implemented in nrGNBFullPHY 
            ...
            for i=1:length(puschInfoList) % For each PUSCH to be received
                ...
                for j=1:length(packetInfoList) % Search PUSCH of interest in the list of received packets
                    ...
                    if (packet.Metadata.PacketType == obj.PXSCHPacketType) && ... % Check for PUSCH
                            any(carrierConfigInfo.NCellID == packet.Metadata.NCellID) && ... % Check for packet of interest
                ...
                if ~isempty(packetOfInterest)
                    ...
                    packetOfInterest.Metadata.NCellID = hPre6GAP.getCPUCellID(obj.APCellID);
        ...
        function decodeSRS(obj, startTime, endTime, carrierConfigInfo)
            ...
            % Read all relevant packets i.e. sent on same
            % carrier frequency and time window
    ...
    methods(Access=protected, Hidden)
        ...
        function [txPkts, rxPkts, controlPktsDL, controlPktsUL] = segregatePackets(~, packets)
            %segregatePackets Segregate packets in the receieved buffer.
            ...
            for i = 1:numel(packets)
                ...
                if strcmp(packetInfo.Metadata.LastTransmitterType, 'CPU')
                    % At CPU DirectID == 1 represents a out-of-band control packet else it is
                    % identified as a in-band data packet
