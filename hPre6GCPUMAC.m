classdef hPre6GCPUMAC < nr5g.internal.nrGNBMAC
    %hPre6GCPUMAC Implements pre6G CPU MAC functionality

    properties(SetAccess=protected)
        %UEsToAPsMap UEsToAPsMap{i} Stores AP node IDs that UE 'i' is connected to
        UEsToAPsMap

        %APsOrderInW Stores the order of AP node IDs used for precoder 'W' computation
        APsOrderInW
    end

    methods
        function obj = hPre6GCPUMAC(param, notificationFcn)
        ...
        function srsIndication(obj, csiMeasurement)
            ...
            % Store the AP order for received srsIndication
            obj.APsOrderInW{csiMeasurement.RNTI} = csiMeasurement.SRSBasedDLMeasurements.APOrder;

            % Update uplink channel measurments (UL MCS, TPMI, UL Rank)
            updateChannelQualityUL(obj.Scheduler, csiMeasurement);

            % Update downlink channel measurments (DL MCS, Precoder, DL Rank)
    ...
    methods(Hidden)
        function updateConnection(obj, ueInfo)
            %updateConnection Updates connection context for connected UEs
